(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        if (typeof window === 'undefined') {
            root.game = module.exports;  // also create global module in Node
        }
    } else {
...
var Board, Cell, Game, count2d, game;
count2d = function flx_count2d (arr, value) {
    var cell, line, stub1_seq, stub2_itr, stub3_seq, stub4_itr, summ;
    summ = 0;
    stub3_seq = arr;
...
Cell.prototype._base_class = Object;
Cell.prototype.__name__ = "Cell";

Cell.prototype.__init__ = function (board, y, x) {
    this.board = board;
    this.y = y;
    this.x = x;
    this.node = document.createElement("div");
    this.board.node.appendChild(this.node);
    this.node.cell = this;
    this.team = 0;
    this.selected = false;
    this.locked = false;
...
Cell.prototype.neighbours = function () {
    var cells, n, nx, ny, stub5_seq, stub6_itr, stub7_seq, stub8_itr, x, y;
    ...
    for (stub8_itr = 0; stub8_itr < stub7_seq.length; stub8_itr += 1) {
        y = stub7_seq[stub8_itr];
        stub5_seq = [-1, 0, +1];
        if ((typeof stub5_seq === "object") && (!Array.isArray(stub5_seq))) { stub5_seq = Object.keys(stub5_seq);}
        for (stub6_itr = 0; stub6_itr < stub5_seq.length; stub6_itr += 1) {
            x = stub5_seq[stub6_itr];
            nx = _pyfunc_op_add(this.x, x);
            ny = _pyfunc_op_add(this.y, y);
            if (((nx >= 0) && ((nx <= (this.board.SIZE - 1))) && (ny >= 0) && ((ny <= (this.board.SIZE - 1))))) {
...
Cell.prototype.set_color = function (color) {
    this.node.style.backgroundColor = color;
    return null;
};

Cell.prototype.onmouseover = function (event) {
    if (_pyfunc_op_equals(this.team, 0)) {
        this.locked = false;
    }
    if (_pyfunc_op_equals(this.locked, false)) {
        if (_pyfunc_op_equals(this.team, 0)) {
            this.set_color("yellow");
        } else if (_pyfunc_op_equals(this.team, 1)) {
            this.set_color("orange");
        } else if (_pyfunc_op_equals(this.team, 2)) {
...
Cell.prototype.onmouseout = function (event) {
    if (_pyfunc_op_equals(this.team, 0)) {
    ...
    if ((_pyfunc_op_equals(this.team, 1) && _pyfunc_op_equals(this.selected, true))) {
        this.set_color("orange");
    } else if ((_pyfunc_op_equals(this.team, 2) && _pyfunc_op_equals(this.selected, true))) {
        this.set_color("green");
    }
    return null;
};
...
Cell.prototype.onclick = function (event) {
    var i, j, stub10_itr, stub11_seq, stub12_itr, stub13_seq, stub14_itr, stub15_seq, stub16_itr, stub9_seq;
    if (_pyfunc_op_equals(game.state, 0)) {
        this.team += 1;
        if ((this.team > 2)) {
            this.team = 0;
        }
    }
    if (_pyfunc_op_equals(game.state, 1)) {
        if ((!_pyfunc_op_equals(this.team, 0))) {
            if ((!_pyfunc_op_equals([this.x, this.y], game.has_moved))) {
                if (_pyfunc_op_equals(this.locked, false)) {
                    if ((_pyfunc_op_equals(this.selected, false) && _pyfunc_op_equals(this.team, game.turn[0]))) {
                        stub11_seq = game.board.cells;
                        ...
                        for (stub12_itr = 0; stub12_itr < stub11_seq.length; stub12_itr += 1) {
                            ...
                            for (stub10_itr = 0; stub10_itr < stub9_seq.length; stub10_itr += 1) {
                                i = stub9_seq[stub10_itr];
                                i.selected = false;
                                i.node.onmouseout();
                            }
                        }
                        this.selected = true;
                    } else if (_pyfunc_op_equals(this.selected, true)) {
                        this.selected = false;
                    }
                }
            }
        } else if (_pyfunc_op_equals(this.team, 0)) {
            stub15_seq = game.board.cells;
            ...
            for (stub16_itr = 0; stub16_itr < stub15_seq.length; stub16_itr += 1) {
                ...
                for (stub14_itr = 0; stub14_itr < stub13_seq.length; stub14_itr += 1) {
                    i = stub13_seq[stub14_itr];
                    if (_pyfunc_op_equals(i.selected, true)) {
                        if ((_pyfunc_op_contains(i.x, [this.x + 1, this.x, this.x - 1]) && _pyfunc_op_contains(i.y, [this.y + 1, this.y, this.y - 1]))) {
                            i.selected = false;
                            this.team = i.team;
                            i.team = 0;
                            i.locked = false;
                            game.turn[1] += 1;
                            if ((game.turn[1] >= 2)) {
                                if (_pyfunc_op_equals(game.turn[0], 1)) {
                                    game.turn[0] = 2;
                                    game.node = document.querySelector("body");
                                    game.node.style.background = "lightblue";
                                } else if (_pyfunc_op_equals(game.turn[0], 2)) {
                                    game.turn[0] = 1;
                                    game.node = document.querySelector("body");
                            ...
                            game.has_moved = [this.x, this.y];
                            i.node.onmouseout();
                        } else if (_pyfunc_op_equals([i.x, i.y], [this.x, this.y - 2])) {
                            console.log("N");
                            if ((((!(_pyfunc_op_equals(i.team, ((game.board.cells[this.y - 1][this.x]).team))))) && ((!(_pyfunc_op_equals(0, ((game.board.cells[this.y - 1][this.x]).team))))))) {
                                i.selected = false;
                                this.team = i.team;
                                i.team = 0;
                                i.locked = false;
                                ...
                                game.has_moved = [this.x, this.y];
                                (game.board.cells[this.y - 1][this.x]).team = 0;
                                ((game.board.cells[this.y - 1][this.x]).set_color)("white");
                                i.node.onmouseout();
                            }
                        } else if (_pyfunc_op_equals([i.x, i.y], [this.x + 2, this.y])) {
                            console.log("O");
                            if ((((!(_pyfunc_op_equals(i.team, ((game.board.cells[this.y][this.x + 1]).team))))) && ((!(_pyfunc_op_equals(0, ((game.board.cells[this.y][this.x + 1]).team))))))) {
                                i.selected = false;
                                this.team = i.team;
                                i.team = 0;
                                i.locked = false;
                                ...
                                game.has_moved = [this.x, this.y];
                                (game.board.cells[this.y][this.x + 1]).team = 0;
                                ((game.board.cells[this.y][this.x + 1]).set_color)("white");
                                i.node.onmouseout();
                            }
                        } else if (_pyfunc_op_equals([i.x, i.y], [this.x, this.y + 2])) {
                            console.log("S");
                            ...
                            if ((((!(_pyfunc_op_equals(i.team, ((game.board.cells[this.y][this.x - 1]).team))))) && ((!(_pyfunc_op_equals(0, ((game.board.cells[this.y][this.x - 1]).team))))))) {
                                ...
                                if ((game.turn[1] >= 2)) {
                                    if (_pyfunc_op_equals(game.turn[0], 1)) {
                                        game.turn[0] = 2;
                                        game.node = document.querySelector("body");
                                        game.node.style.background = _pymeth_format.call("lightblue");
                                    } else if (_pyfunc_op_equals(game.turn[0], 2)) {
                                        game.turn[0] = 1;
                                        game.node = document.querySelector("body");
                                        game.node.style.background = "pink";
...
Board.prototype._base_class = Object;
Board.prototype.__name__ = "Board";

Board.prototype.SIZE = 32;
Board.prototype.__init__ = function () {
    var row, x, y;
    this.node = document.querySelector("body");
    this.node.style.background = _pymeth_format.call("lightgray");
    this.node = document.querySelector("#board");
    this.node.style.gridTemplateColumns = _pymeth_format.call("repeat({}, 1fr)", this.SIZE);
    this.cells = [];
    for (y = 0; y < this.SIZE; y += 1) {
        row = [];
        for (x = 0; x < this.SIZE; x += 1) {
            _pymeth_append.call(row, (new Cell(this, y, x)));
        }
...
Game.prototype._base_class = Object;
Game.prototype.__name__ = "Game";

Game.prototype.__init__ = function () {
    this.board = new Board();
    this.button_start_game = document.querySelector("#start_game");
    this.button_start_game.onclick = this.on_start_game;
    this.state = 0;
    this.has_moved = null;
    this.turn = [0, 0];
    return null;
...
Game.prototype.on_start_game = function (event) {
    if (_pyfunc_op_equals(this.state, 0)) {
        this.state = 1;
        this.turn = [1, 0];
        game.node = document.querySelector("body");
        game.node.style.background = "pink";
...
Game.prototype.end_turn = function () {
    var cell, line, stub17_seq, stub18_itr, stub19_seq, stub20_itr;
    console.log("end_turn");
    stub19_seq = this.board.cells;
    ...
    for (stub20_itr = 0; stub20_itr < stub19_seq.length; stub20_itr += 1) {
        ...
        for (stub18_itr = 0; stub18_itr < stub17_seq.length; stub18_itr += 1) {
            cell = stub17_seq[stub18_itr];
            if ((!_pyfunc_op_equals(cell.team, 0))) {
                if (((count2d(cell.neighbours(), cell.team)) < 1)) {
                    cell.team = 0;
                    cell.locked = false;
                    cell.set_color("white");
                }
                if (_pyfunc_op_equals(cell.team, 1)) {
                    if (((count2d(cell.neighbours(), 2)) >= 4)) {
                        cell.team = 0;
                        cell.locked = false;
                        cell.set_color("white");
                    } else if ((_pyfunc_op_equals((count2d(cell.neighbours(), 2)), 3))) {
                        cell.locked = true;
                        cell.set_color("#800");
                    } else {
                        cell.locked = false;
                ...
                } else if (_pyfunc_op_equals(cell.team, 2)) {
                    if (((count2d(cell.neighbours(), 1)) >= 4)) {
                        cell.team = 0;
                        cell.locked = false;
                        cell.set_color("white");
                    } else if ((_pyfunc_op_equals((count2d(cell.neighbours(), 1)), 3))) {
                        cell.locked = true;
                        cell.set_color("#008");
                    } else {
                        cell.locked = false;
...
return {count2d: count2d, Cell: Cell, game: game, Game: Game, Board: Board};
})); 
